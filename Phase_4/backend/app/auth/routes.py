...
async def login(
    ...
    print(f"DEBUG: Raw request body: {body_bytes}")
    ...
    print(f"DEBUG: Received email: '{email}', password length: {len(password)}")

    if not email or not password:
        print(f"DEBUG: Validation failed - email empty: {not email}, password empty: {not password}")
    ...
    try:
        ...
        print(f"DEBUG: User lookup result - found: {user is not None}")

        if not user or not verify_password(password, user.password_hash):
            print(f"DEBUG: Authentication failed - user exists: {user is not None}, password valid: {user and verify_password(password, user.password_hash) if user else False}")
