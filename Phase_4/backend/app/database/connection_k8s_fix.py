...
# Initialize async components (use the cleaned URL immediately)
async_engine = None
...
if is_postgres:
    logger.info("=" * 80)
    logger.info("🚀 MCP SERVER: Building async database URL with SSL disabled...")
    database_url_async = build_clean_asyncpg_url(settings.database_url)
    logger.info(f"🔧 MCP SERVER: Cleaned URL: {database_url_async}")
    logger.info("=" * 80)
    ...
    logger.info("✅ MCP SERVER: Async engine and session maker initialized with cleaned URL")
else:
