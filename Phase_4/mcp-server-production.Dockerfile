...
# Copy the MCP server files - with explicit SSL disable fix for Kubernetes
...
# CRITICAL: Copy the Kubernetes-optimized database connection file that forces sslmode=disable
COPY backend/app/database/connection_k8s_fix.py /app/app/database/connection.py
