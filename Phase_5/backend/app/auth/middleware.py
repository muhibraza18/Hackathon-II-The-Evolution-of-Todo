...
def get_current_user_id(request: Request) -> int:
    """
    Helper function to get the current user_id from request state.
    This can be used in route handlers that need the authenticated user ID.
    """
    # Get user_id from request state (set by auth middleware)
    user_id = getattr(request.state, 'user_id', None)
    if not user_id:
        # For development, return default user_id
        # In production, this should raise an exception
        return 1
    return user_id


async def get_current_user_id_dependency(request: Request) -> int:
    """
    Async dependency function to get the current user_id from request state.
    This can be used in route handlers that need the authenticated user ID.
    """
    # Get user_id from request state (set by auth middleware)
    user_id = getattr(request.state, 'user_id', None)
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def _is_public_endpoint(self, path: str) -> bool:
        ...
        prefix_paths = [
            ...
            "/redoc",
            "/openapi.json",
            "/dapr/config",      # Dapr sidecar configuration endpoint
            "/dapr/subscribe",   # Dapr pub/sub subscription endpoint
            "/dapr/subscribe",   # Dapr binding subscription endpoint
        ]
