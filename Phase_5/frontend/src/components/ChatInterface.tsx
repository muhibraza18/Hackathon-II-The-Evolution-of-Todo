...
// Format message content to make it more readable
const formatMessageContent = (content: string) => {
  ...
  lines.forEach((line, index) => {
    ...
    // Check if line is a task item (starts with number, bullet, or dash)
    const isTaskItem = /^[\d]+[\.\)]\s/.test(trimmedLine) || 
                       /^[-•*]\s/.test(trimmedLine) ||
                       /^Task \d+:/i.test(trimmedLine);
    ...
    if (isTaskItem) {
      ...
      if (inList && listItems.length > 0) {
        formatted.push(
          <div key={`list-${index}`} className="task-list">
      ...
      if (trimmedLine) {
        ...
        if (trimmedLine.match(/^(Here are|Your tasks|Tasks:|Upcoming tasks)/i)) {
          formatted.push(
            <div key={`header-${index}`} className="message-header">
              {trimmedLine}
            </div>
          );
        } else {
  ...
  if (inList && listItems.length > 0) {
    formatted.push(
      <div key="list-final" className="task-list">
        {listItems.map((item, i) => (
          <div key={i} className="task-item">
            <div className="task-bullet"></div>
            <span className="task-text">{item}</span>
  ...
  return formatted.length > 0 ? formatted : <span>{content}</span>;
};
...
const ChatInterface: React.FC = () => {
  ...
  useEffect(() => {
    for (const reminder of reminders) {
      const reminderKey = `${reminder.task_id}-${reminder.due_date}`;

      if (!notifiedReminderIds.has(reminderKey)) {
